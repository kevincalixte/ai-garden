...
const Card = ({ type = "plant", title, text, img }: CardProps) => {
  const cardStyles = {
    ...
    tip: {
      style: "max-h-25 w-[48%] bg-[#074703]/90",
    },
    suggestion: {
  ...
  return (
    <div
      className={`
        ${baseStyle} rounded-xl text-white mt-4 ${currentStyle.style}`}
      style={img ? { backgroundImage: `url(/assets/img${img})` } : undefined}
    >
      <div className="flex flex-col justify-center p-3">
        {title && <h2 className="text-xl font-semibold mb-2 line-clamp-2">{title}</h2>}
